...
export class AgentSidebarView extends ItemView {
    ...
    private buildHeader(container: HTMLElement): void {
        ...
        const titleChevron = titleRow.createSpan('agent-title-chevron');
        ...
        // New chat button — first in header-right (Notion style)
        ...
        // Vault-health indicator (hidden unless at least one finding exists)
    ...
    private renderTabStrip(): void {
        ...
        // Minimalist UX: hide tab bar when only 1 chat session is open
        if (this.sessions.length <= 1) {
            strip.addClass('agent-u-hidden');
            return;
        }
    ...
    /** Suggestion banner is disabled from the chat sidebar to keep the interface clean. */
    private buildSuggestionBanner(_container: HTMLElement): void {
        // Disabled
    }
    ...
    private showWelcomeMessage(): void {
        ...
        // Render modern Notion-style empty state if container is empty
        if (this.chatContainer && this.chatContainer.childElementCount === 0) {
            this.renderEmptyState(this.chatContainer);
        }
    }
    ...
    private renderEmptyState(container: HTMLElement): void {
        ...
        for (const action of actions) {
            const item = listContainer.createDiv('nova-notion-action-item');
    ...
    private addUserMessage(
        ...
        const target = container ?? this.chatContainer;
        if (!target) return;
        target.querySelector('.nova-empty-state')?.remove();
