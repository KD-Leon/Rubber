...
export class ProvidersTab {
    ...
    build(containerEl: HTMLElement): void {
        ...
        // Section description
        const intro = containerEl.createDiv({ cls: 'nova-settings-section-desc' });
        intro.setText(t('settings.providers.intro'));
    ...
    private rowSummary(provider: ProviderConfig): string {
        ...
        const flagship = this.tierShortLabel(provider, 'flagship');
        const mid = this.tierShortLabel(provider, 'mid');
        const fast = this.tierShortLabel(provider, 'fast');

        const primaryModel = (mid && mid !== '—') ? mid : ((flagship && flagship !== '—') ? flagship : ((fast && fast !== '—') ? fast : null));
        if (primaryModel) {
            return `主力模型: ${primaryModel} · 共 ${count} 个可用模型`;
        }
        return `已配置 ${count} 个模型`;
    }
